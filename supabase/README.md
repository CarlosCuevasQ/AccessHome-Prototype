...
2. En SQL Editor, ejecutar los ocho archivos base en el orden indicado en [SHARED_BACKEND_SETUP.md](../docs/SHARED_BACKEND_SETUP.md) y después las incrementales de caseta `20260917000600`, consulta pública `20260917000700` y escaneo `20260918000100`. Cada archivo tiene `begin/commit`: si falla, corregir la causa y reintentar solo la versión que no se confirmó.
3. Registrar las once versiones aplicadas y el proyecto, sin guardar contraseñas ni claves en Git. No editar y repetir una migración ya aplicada: las correcciones posteriores requieren una nueva versión.
...
3. Revisar `supabase migration list` y `supabase db push --dry-run`. En un proyecto nuevo verificar las once versiones; en uno existente, solo las incrementales aún no aplicadas.
...
- Las funciones tienen autorización y grants específicos. Guardia consulta caseta e invoca validate_access del propio condominio; no administra ni escribe tablas. Servicios, reportes de turno, Realtime e importación local siguen pendientes. Escaneo remoto requiere la nueva incremental y aceptación manual.
