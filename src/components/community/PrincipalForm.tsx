...
export function PrincipalForm({ data, onSaved, onCancel }: { data: ResidenceDetails; onSaved: () => void; onCancel: () => void }) {
  const candidates = data.inhabitants.filter((person) => person.active)
  ...
  return (
    <EditorForm title="Asignar residente principal" save={save} onSaved={onSaved} onCancel={onCancel}>
      <label className="form-field">Asignación<select value={mode} onChange={(event) => { setMode(event.target.value); setEmail('') }}>
        ...
        <option value="new">Nuevo residente principal</option>
      </select></label>
      ...
      <p className="form-help">{needsAccount ? 'Se creará una cuenta con este correo y la contraseña de demostración indicada en el login.' : 'Se conservará la cuenta de acceso de este habitante.'} El principal anterior permanecerá en la casa como habitante y dejará de poder gestionarla. No se trasladan personas desde otras casas.</p>
    </EditorForm>
  )
}
