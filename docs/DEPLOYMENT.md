...
Código preparado y probado localmente. **No se creó un deployment, no se aplicó SQL remoto ni se modificó `.env.local`.** La nueva migración `20260917000700_public_invitation_sharing.sql` está pendiente de aplicación por el responsable. Las ocho iniciales ya están aplicadas según su confirmación; verificar por separado si la novena, de Guardia, ya figura en el historial. No repetir versiones ni ejecutar `seed_demo` o reset.
...
4. Aplicar **solo la incremental pendiente** [20260917000700_public_invitation_sharing.sql](../supabase/migrations/20260917000700_public_invitation_sharing.sql), completa, desde SQL Editor como propietario controlado o mediante el procedimiento CLI revisado. Tiene transacción propia; no actualiza filas, tokens, cuentas ni invitaciones.
5. Ejecutar la auditoría de solo lectura [security_baseline.sql](../supabase/tests/security_baseline.sql). Debe finalizar sin excepciones. Mantener únicamente `accesshome` expuesto para la app; `accesshome_private` debe seguir fuera de Data API y Extra search path.
6. Ejecutar `npm run backend:check` desde el equipo configurado. Debe informar base AccessHome, esquema 9, `guardWorkspaceVersion: 1` y **`publicInvitationVersion: 2`**. El número de esquema base se conserva por compatibilidad; no cuenta archivos de migración. Este chequeo no prueba login ni navegadores y no se ejecutó remotamente en esta entrega.

La incremental sustituye la proyección del RPC público: visitante, casa, condominio, fechas, estado y el mismo token. Retira del resultado público anfitrión, vehículo, usos y permiso de edición; no expone teléfono o correo. Conserva la firma del RPC, el límite de frecuencia, los bloqueos y su operación opcional de vehículo para clientes anteriores. La pantalla pública de esta etapa muestra únicamente la consulta solicitada; ya no presenta el formulario de vehículo. El residente sigue preparando sus datos al crear la invitación. Aplicar backend y frontend en la misma ventana de prueba, porque la proyección anterior contenía más campos.
